...
private fun checkForRange(input: List<DayFourRange>, isOverlapNeeded: Boolean = false): List<Int> {
    val listInRange = mutableListOf<Int>()
    val listOverlap = mutableListOf<Int>()
    ...
    input.forEach {
        ...
        listInRange.add(if (range1WithinRange2 || range2WithinRange1) 1 else 0)
        listOverlap.add(if (overlapToLeft || overlapToRight) 0 else 1)
    }

    return if (isOverlapNeeded) listOverlap else listInRange
}
private fun readDayFourData(input: String): List<DayFourRange> {
